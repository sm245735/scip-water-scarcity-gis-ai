...
已在本機建立 PostgreSQL/PostGIS、恢復 CSV 並完成資料品質盤點。
請先閱讀 [復工操作與驗收報告](doc/復工實作_20260919.md)。
下方為四月研究紀錄；六月研究以 **7／14 天後蓄水率** 為目標，實驗尚待重現。
目前恢復資料庫使用 `docker compose --env-file .env.recovery`，不使用下方舊容器名稱。
...
| **調度資料限制** | 寶二為離槽水庫，蓄水變化涉及引水與調度；缺少相關變數是模型限制，不能視為純自然水文 |
